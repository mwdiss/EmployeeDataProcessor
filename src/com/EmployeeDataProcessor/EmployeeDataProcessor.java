...
class CSVLoader { //parses CSV data with a built-in encoding fallback
    public static LoadResult loadWithFallback(File file, List<Charset> charsetsToTry) throws Exception {
        ...
        for (Charset charset : charsetsToTry) { //attempts to read
            try { rawLines = Files.readAllLines(file.toPath(), charset); break; /*succeeds*/ } catch (java.nio.charset.MalformedInputException ignored) { /*failed, try next*/ } }
        if (rawLines == null || rawLines.isEmpty()) throw new Exception("Could not decode file. Try a different encoding type.");

        //continue parsing with the successfully decoded lines.
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            try { String name=nameIdx!=-1?p[nameIdx].trim().replace("\"",""):"N/A", dept=p[deptIdx].trim().replace("\"",""); empList.add(new Employee(name, Integer.parseInt(p[ageIdx].trim()), dept, Double.parseDouble(p[salIdx].replaceAll("[^\\d.]","").trim())));
    ...
    } public static int findHIdx(String[] h, String... n) { return IntStream.range(0, h.length).filter(i->Arrays.stream(n).anyMatch(h[i].trim().toLowerCase()::contains)).findFirst().orElse(-1); } //helper to find a header index by keywords
}
...
class ImportFrame extends JFrame { private static final long serialVersionUID = 1L;
    ...
    private final JComboBox<String> encodingBox = new JComboBox<>(new String[]{"UTF-8", "ISO-885ทท"});
    public ImportFrame() {
...
class MainFrame extends JFrame { private static final long serialVersionUID = 1L;
    ...
    private void runAssignmentOps() { //main assignment logic using streams & function
        ...
        /*3. filter by age*/
        txt.append("\n2. Employees > 30: ").append(allEmployees.stream().filter(e->e.age()>30).count()).append(" of ").append(allEmployees.size()); txt.append("\n3. First 10 Employees (Sorted by Name):\n");
    ...
    private void applyFilters() { //combines quick search and advanced filters
        ...
        if(!q.isEmpty()){ String p = q.startsWith("\"")&&q.endsWith("\"")?"(?i)^"+Pattern.quote(q.substring(1,q.length()-1))+"$":"(?i)"+Pattern.quote(q); var pat=Pattern.compile(p); 
        //exact match for quoted text, otherwise contains
        filters.add(new RowFilter<>(){public boolean include(Entry<?,?>e){ for(int i=0;i<table.getColumnCount();i++) if(e.getValue(table.convertColumnIndexToModel(i))!=null&&pat.matcher(e.getValue(table.convertColumnIndexToModel(i)).toString()).find())return true; return false;}}); } 
...
class ColumnToggleDialog extends JDialog { /*column hide/show/reorder management*/ private static final long serialVersionUID = 1L;
    ...
    public ColumnToggleDialog(JFrame p, String[] h, JTable tbl) { //builds list of all columns, marking currently visible ones
        super(p,"Columns",true); table=tbl; setLayout(new BorderLayout(10,10));
    ...
    private void apply(){TableColumnModel cm=table.getColumnModel(); while(cm.getColumnCount()>0)cm.removeColumn(cm.getColumn(0));IntStream.range(0,listModel.size()).mapToObj(listModel::get).filter(s->s.checkBox().isSelected()).forEach(s->{s.column().setModelIndex(s.modelIndex());s.column().setHeaderValue(s.header());cm.addColumn(s.column());});
    UIHelper.adjustColumnWidths(table);table.getTableHeader().revalidate();table.repaint();dispose();} //rebuilds table column model based on user choices
}

class FilterDialog extends JDialog { /*stateful, reusable filter dialog*/ private static final long serialVersionUID = 1L;
    public FilterDialog(JFrame p, String[] h, Map<Integer, List<String>> cM, Map<Integer,Object[]> state, Consumer<FilterResult> onApply) { //dynamically builds filter UI and pre-populates from saved state
    ...
    private void apply(String[] h, Map<Integer,List<String>> cM, List<JComponent> cL, Consumer<FilterResult> onApply) { //constructs complex filter and saves UI state from components
...
public class EmployeeDataProcessor { //main entry point
    public static void main(String[] args) { 
    	...
    	catch(Exception e){e.printStackTrace();} SwingUtilities.invokeLater(()->new ImportFrame().setVisible(true)); 
}  }